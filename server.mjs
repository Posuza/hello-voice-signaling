...
const server = http.createServer((req, res) => {
  ...
  res.writeHead(200);
  res.end(JSON.stringify({ ok: true, service: "hello-voice-signaling", protocol: 20, mode: "distributed-p2p-relay", maxSpeakers: MAX_SPEAKERS, maxListeners: MAX_LISTENERS, listenersPerRelay: LISTENERS_PER_RELAY }));
});
...
wss.on("connection", ws => {
  ...
  ws.on("message", raw => {
    ...
    if (msg.type === "force-mute") {
      ...
      updateMember(room, peerId, { speaking: false });

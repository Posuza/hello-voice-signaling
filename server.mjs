...
const GAME_COLORS = ["#20c77a", "#4f8de8", "#f0a43c", "#b466d4", "#e45f73"];
...
function gameRoomPublic(room) {
  return {
    ...
    players: [...room.players.values()].map(p => ({ userId:p.userId, name:p.name, ready:p.ready, isHost:p.userId === room.hostUserId, connected:p.connected, color:p.color })),
...
function createKingdomMatch(room) {
  ...
  const spawns = [[260,260],[2140,260],[2140,1340],[260,1340],[1200,210]];
  const state = {
    matchId, roomId:room.id, gameId:"kingdom", state:"PLAYING", world:{width:2400,height:1600}, startedAt:Date.now(), winnerId:null,
    players: players.map((p,i)=>({userId:p.userId,name:p.name,alive:true,resources:{gold:320,wood:280,food:220},color:GAME_COLORS[i%GAME_COLORS.length]})),
    units:[], buildings:[], territories:[
      {id:"center",x:1200,y:800,radius:120,ownerId:null,kind:"central"},
      {id:"gold-nw",x:720,y:520,radius:85,ownerId:null,kind:"gold"}, {id:"wood-ne",x:1680,y:520,radius:85,ownerId:null,kind:"wood"},
      {id:"food-sw",x:720,y:1080,radius:85,ownerId:null,kind:"food"}, {id:"gold-se",x:1680,y:1080,radius:85,ownerId:null,kind:"gold"}
    ], stats:{}
  };
  players.forEach((p,i)=>{
...
function handleGameMessage(ws,msg){
  ...
  if(type==="game:create"){
    if(!userId)return send(ws,{type:"game:error",message:"Sign in first"})||true; const id=crypto.randomUUID();const room={id,gameId,name:String(msg.roomName||"Kingdom Room").trim().slice(0,50)||"Kingdom Room",hostUserId:userId,state:"WAITING",mapId:"green-valley",mode:"free-for-all",minPlayers:2,maxPlayers:5,players:new Map(),matchId:null,createdAt:Date.now()};
    room.players.set(userId,{userId,name,ready:false,connected:true,ws,color:GAME_COLORS[0],disconnectedAt:null});gameRooms.set(id,room);gameConnections.set(ws,{gameId,roomId:id,userId});send(ws,{type:"game:room-snapshot",room:gameRoomPublic(room)});broadcastGameRoomList(gameId);return true;
  }
  if(type==="game:join"||type==="game:reconnect"){
    ...
    let p=room.players.get(userId);if(!p){if(room.state!=="WAITING")return send(ws,{type:"game:error",message:"Match already started"})||true;if(room.players.size>=room.maxPlayers)return send(ws,{type:"game:error",message:"Game room is full"})||true;p={userId,name,ready:false,connected:true,ws,color:GAME_COLORS[room.players.size%GAME_COLORS.length],disconnectedAt:null};room.players.set(userId,p);}else{p.ws=ws;p.connected=true;p.disconnectedAt=null;p.name=name||p.name;}
...
const server = http.createServer((req, res) => {
  ...
  res.writeHead(200);
  res.end(JSON.stringify({ ok: true, service: "hello-voice-signaling", protocol: 52, mode: "p2p-conversation-caster-relay", maxSpeakers: MAX_SPEAKERS, maxListeners: MAX_LISTENERS, listenersPerRelay: LISTENERS_PER_RELAY, maxRelays: MAX_RELAYS }));
});

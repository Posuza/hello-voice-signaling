...
const server = http.createServer((req, res) => {
  ...
  res.writeHead(200);
  res.end(JSON.stringify({ ok: true, service: "hello-voice-signaling", protocol: 31, mode: "distributed-p2p-relay", maxSpeakers: MAX_SPEAKERS, maxListeners: MAX_LISTENERS, listenersPerRelay: LISTENERS_PER_RELAY }));
});
...
wss.on("connection", ws => {
  ...
  ws.on("message", raw => {
    ...
    if (msg.type === "join") {
      ...
      if (!room) {
        room = { peers: new Map(), messages: [], hostPeerId: null, nextJoinOrder: 1 };
      ...
      const user = {
        id: peerId, name, role, relayPeerId, speaking: false,
